...
class DownloadService:
    ...
    async def save_content(
        ...
        Args:
            ...
            is_stream: Whether to treat content as a stream (for large files)
        ...
        try:
            ...
            if is_stream:
                # Stream content directly to file for large files
            ...
            raise DownloadError(f"Unexpected error saving file {destination}: {e}")
    ...
    async def save_content_with_progress(
        ...
        def progress_callback(chunk_bytes: int, total_bytes: int) -> None:
            progress_info.update_file_progress(chunk_bytes, filename)

        config = config or DownloadConfig(progress_callback=progress_callback)

        bytes_written = await self.save_content(
            content=content, destination=destination, config=config
        )
    ...
    async def directory_exists(self, path: Path) -> bool:
        ...
        try:
            return await asyncio.to_thread(lambda: path.exists() and path.is_dir())
        except Exception:
            return False
    ...
    async def get_file_size(self, path: Path) -> int:
        ...
        try:
            ...
            raise DownloadError(f"Failed to get file size for {path}: {e}")
    ...
    async def delete_file(self, path: Path) -> bool:
        ...
        try:
            ...
            raise DownloadError(f"Failed to delete file {path}: {e}")
    ...
    async def create_backup(self, path: Path) -> Path:
        ...
        if not await self.file_exists(path):
            raise DownloadError(f"Cannot backup non-existent file: {path}")
    ...
    async def batch_save_contents(
        ...
        async def save_with_semaphore(content: bytes, path: Path) -> int:
            async with semaphore:
                return await self.save_content(content, path, show_progress=False)

        # Create tasks for all save operations
        tasks = [
            save_with_semaphore(content, path) for content, path in contents_and_paths
        ]
        ...
        if show_progress:
            results = []
            progress_bar = async_tqdm(
                total=len(tasks), desc="Saving files", unit="file"
            )
    ...
    async def cleanup_temp_files(self, directory: Path, pattern: str = "*.tmp") -> int:
        """
        ...
        try:
            temp_files = await asyncio.to_thread(lambda: list(directory.glob(pattern)))

            cleanup_tasks = [self.delete_file(temp_file) for temp_file in temp_files]
